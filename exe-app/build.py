...
def build_executable():
    ...
    exe_path = OUTPUT_DIR / 'AutoGLM.exe'
    if exe_path.exists():
...
def create_portable_package():
    ...
    zip_name = f"AutoGLM-Portable-{timestamp}.zip"
    zip_path = OUTPUT_DIR / zip_name
    
    # 需要打包的文件
    files_to_pack = [
        (OUTPUT_DIR / 'AutoGLM.exe', 'AutoGLM.exe'),
        (EXE_APP_DIR / 'README.md', 'README.md'),
    ]
    
    # 可选：打包 ADBKeyboard.apk
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        # 创建示例 .env 文件
        env_content = """# AutoGLM 配置文件
# 请填入你的 API Key

# 智谱 AI API Key (用于手机控制)
API_KEY=your_zhipu_api_key_here

# 硅基流动 API Key (用于聊天功能，可选)
SILICONFLOW_API_KEY=your_siliconflow_api_key_here
"""
        zf.writestr('.env.example', env_content)
        print("   添加: .env.example")
...
def print_summary():
    ...
    print("\n使用说明:")
    print("  1. 解压 AutoGLM-Portable-*.zip 到任意目录")
    print("  2. 将 .env.example 复制为 .env 并配置 API Key")
    print("  3. 双击 AutoGLM.exe 运行")
    print()
