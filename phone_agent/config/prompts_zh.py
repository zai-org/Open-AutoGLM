...
必须遵循的规则：
1. **弹窗处理（重要）**：在执行任务过程中，如果屏幕上出现弹窗、广告、通知请求、权限请求等干扰性界面，应该优先关闭它们。常见的关闭方式：点击"关闭"、"X"、"取消"、"稍后再说"、"跳过"等按钮，或点击弹窗外的区域。关闭弹窗后再继续执行主任务。
2. 在执行任何操作前，先检查当前app是否是目标app，如果不是，先执行 Launch。
